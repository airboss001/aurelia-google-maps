...
export class GoogleMaps
{
  ...
  private configRenderPolygons: any;
  private configRenderPolylines: any;
  ...
  )
  {
    ...
    this.configRenderPolygons = {
      ...
      sub: null,
      collection: [],
      renderType: Events.POLYGON,
      polyClick: Events.POLYGONCLICK,
      polyRendered: Events.POLYGONRENDERED
    };
    ...
    this.configRenderPolylines = {
      ...
      sub: null,
      collection: [],
      renderType: Events.POLYLINE,
      polyClick: Events.POLYLINECLICK,
      polyRendered: Events.POLYLINERENDERED
    };
  ...
  zoomToMarkerBounds(force = false)
  {
    ...
    this._mapPromise.then(() =>
    {
      ...
      if (this.configRenderPolygons.collection.length >= 1)
      {
        for (let polygon of this.configRenderPolygons.collection)
        {
      ...
      if (this.configRenderPolylines.collection.length >= 1)
      {
        for (let polygon of this.configRenderPolylines.collection)
        {
          polygon.getPath().forEach(element =>
          {
            bounds.extend(element);
  ...
  {
    this.polyChanged(newValue, Events.POLYGON);
  }

  polylinesChanged(newValue: any)
  {
    this.polyChanged(newValue, Events.POLYLINE);
  }

  polyChanged(newValue: any, type: Events)
  {
    let configRender: any;

    if (type === Events.POLYGON)
    {
      configRender = this.configRenderPolygons;
    }
    else if (type === Events.POLYLINE)
    {
    ...
    if (configRender.sub !== null)
    {
      ...
      for (let poly of configRender.collection)
      {
        poly.setMap(null);
        poly = null;
      }
    ...
    configRender.sub = this.bindingEngine
      ...
      .subscribe((splices) => { this.collectionChange(splices, configRender); });
    ...
    this._mapPromise.then(() =>
    {
      Promise.all(
        newValue.map(poly =>
        {
          ...
          {
            return this.decodePath(poly);
          }
          return poly;
        })
      ).then(poly =>
      {
        return Promise.all(poly.map((e) =>
        {
          this.renderPoly(e, configRender);
        }));
      }).then(() =>
      {
        /**
   ...
   * @param splices
   */
  collectionChange(splices: any, config: any)
  {
    if (!splices.length)
    {
      // Collection changed but the splices didn't
    ...
    this._mapPromise.then(() =>
    {
      for (let splice of splices)
      {
        if (splice.removed.length)
        {
          ...
          {
            // Iterate over all the rendered markers to find the one to remove
            for (let polyIndex in config.collection)
            {
              ...
              let renderedPoly = config.collection[polyIndex];

              // Get string representation
              let strRendered, strRemoved;

              strRendered = this.encodePath(renderedPoly.getPath());
              ...
              // Splice out this rendered marker as well
              config.collection.splice((<any>polyIndex), 1);
              break;
            }
          }
        }
        ...
        if (splice.addedCount)
        {
          ...
          {
            this.renderPoly(addedPolygon, config);
          }
        }
      }
    }).then(() =>
  ...
  /**
   * Render a single polyline on the map and add it to the _renderedPolylines
   * array.
   * @param polygonObject - path defining a polyline or a string
   */
  renderPoly(polyObject: any = [], config: any)
  {
    let path = polyObject.paths;
    ...
    if (config.renderType === Events.POLYGON)
    {
      poly = new (<any>window).google.maps.Polygon(
        Object.assign({}, polyObject, { path })
      );
    }
    else if (config.renderType === Events.POLYLINE)
    {
      poly = new (<any>window).google.maps.Polyline(
        Object.assign({}, polyObject, { path })
      );
    }
    else
    ...
    {
      dispatchEvent(config.polyClick, { poly }, this.element);
    });
    ...
    {
      poly.infoWindow = new (<any>window).google.maps.InfoWindow({
        ...
        parentPolygon: { ...polyObject }
      });
    }

    dispatchEvent(config.polyRendered, { poly, polyObject }, this.element);
